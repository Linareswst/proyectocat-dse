...
namespace ProyectoHotel.Logica
{
    public class ReporteLogica
    {
        ...
        public DataTable Accidentes(string estado, string fechainicio, string fechafin)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    ...
                    sb.AppendLine("SET DATEFORMAT dmy;");
                    sb.AppendLine("SELECT h.Detalle, p.Descripcion AS TipoAccidente, c.Descripcion AS EscalaGravedad, IIF(h.Estado = 1, 'Activo', 'No Activo') AS Estado, CONVERT(char(10), h.FechaCreacion, 103) AS [Fecha Creacion]");
                    sb.AppendLine("FROM habitacion h");
                    sb.AppendLine("INNER JOIN Piso p ON h.IdPiso = p.IdPiso");
                    sb.AppendLine("INNER JOIN Categoria c ON h.IdCategoria = c.IdCategoria");
                    sb.AppendLine("WHERE h.Estado = IIF(@estado = 2, h.Estado, @estado) AND CONVERT(date, h.FechaCreacion) BETWEEN @fechainicio AND @fechafin");
